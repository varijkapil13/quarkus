...
:categories: web
:summary: Discover how to develop highly scalable reactive REST services with JAX-RS and RESTEasy Reactive.
:jaxrsapi: https://javadoc.io/doc/jakarta.ws.rs/jakarta.ws.rs-api/3.1.0
...
You can also use any of the JAX-RS annotations link:{jaxrsapi}/jakarta/ws/rs/PathParam.html[`@PathParam`],
...
NOTE: You can also use the JAX-RS type link:{jaxrsapi}/jakarta/ws/rs/core/Response.html[`Response`] but it is
...
https://javadoc.io/static/jakarta.inject/jakarta.inject/1/javax/inject/Inject.html[`@Inject`] on fields of the same
type:
...
Assuming security has been set up for the application (see our xref:security.adoc[guide] for more details), when a user with the `admin` role
