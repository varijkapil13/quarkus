...
You can obtain a fragment programatically via the `io.quarkus.qute.Template.getFragment(String)` method.
...
TIP: In Quarkus, it is also possible to define a <<type_safe_fragments,type-safe fragment>> via the `@CheckedFragment` annotation.
